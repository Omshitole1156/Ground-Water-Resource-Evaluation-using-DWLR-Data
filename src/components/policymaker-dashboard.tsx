...
export default function PolicymakerDashboard({
  ...
  const {
    ...
    const maharashtraStations = stations.filter(
      (s) => s.state === "Maharashtra",
    );
    ...
    maharashtraStations.forEach((station) => {
      if (!districtStatusMap[station.district]) {
        districtStatusMap[station.district] = {
    ...
    const topMaharashtraCities = maharashtraStations
      ...
      .sort(
        (a, b) =>
          (populations[b.district] || 0) - (populations[a.district] || 0),
      )
      .slice(0, 6);

    const rankedCities = maharashtraStations
      .map((station) => {
        const preMonsoon = station.timeSeries.find(
          (d) => new Date(d.date).getMonth() === 4,
        ); // May
        const postMonsoon = station.timeSeries.find(
          (d) => new Date(d.date).getMonth() === 10,
        ); // November
        let change = 0;
        if (preMonsoon && postMonsoon) {
          change =
            ((postMonsoon.level - preMonsoon.level) / preMonsoon.level) * 100;
        }
  ...
  const policySummaryPoints = [
    {
      title: "Top Cities with Declining Trends",
      content:
        "Nagpur, Pune, and Nashik show the most significant 10-year declining groundwater trends, indicating severe long-term stress. Immediate demand-side management is crucial.",
      icon: <TrendingUp className="text-destructive" />,
    },
    {
      title: "Strong Post-Monsoon Recovery",
      content:
        "Stations in Aurangabad and Solapur show strong post-monsoon recovery, suggesting effective natural recharge in these basalt aquifer regions.",
      icon: <TrendingUp className="text-green-500" />,
    },
    {
      title: "Suggested Local Interventions",
      content:
        "For hard-rock aquifers in Pune and Nashik, prioritize artificial recharge through injection wells and check dams. In alluvial areas of Nagpur, promote rainwater harvesting.",
      icon: <Lightbulb />,
    },
  ];
    ...
    <Tabs defaultValue="overview">
      ...
      <TabsContent value="maharashtra" className="mt-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="flex gap-4">
                ...
                <Button variant="outline">
                  <ChevronsRight className="mr-2 h-4 w-4" /> Sample JSON
                </Button>
              </CardContent>
            </Card>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Groundwater Trends: Top 6 Cities by Population
            </CardTitle>
            <CardDescription>
              Time-series data for the last 5 years.
            </CardDescription>
          </CardHeader>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Number of stations in Critical, Warning, and Normal status for
                each district.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Ranked list of cities by % groundwater level change.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                  ...
                  {rankedCities.slice(0, 5).map((city) => (
                    <TableRow key={city.id}>
                      ...
                      <TableCell className="text-right">
                        <Badge
                          variant={city.change > 0 ? "default" : "destructive"}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              AI-suggested strategies to improve groundwater levels.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <RechartsBarChart
                data={graphicStrategies.labels.map((label, index) => ({
                  name: label,
                  value: graphicStrategies.data[index],
                }))}
                layout="vertical"
              >
                <XAxis type="number" />
